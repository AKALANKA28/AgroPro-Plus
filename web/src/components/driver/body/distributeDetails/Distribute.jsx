...
function Distribute() {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://127.0.0.1:8000/distribute/delete/${id}`);
...
const handleAddSubmit = async (formData) => {
  try {
    ...
    const response = await axios.post("http://127.0.0.1:8000/distribute/add", formData);
  ...
  const handleEditSubmit = async (formData) => {
    try {
      await axios.patch(`http://127.0.0.1:8000/distribute/update/${formData._id}`, formData);
